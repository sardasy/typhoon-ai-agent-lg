...
- All 117 tests pass without any external dependencies (no API key, no HIL, no XCP)
...
```bash
# Run tests (117 tests, should all pass — run this after every change)
...
2. **Run tests after every change.** `python -m pytest tests/ -v` must stay at
   117 passed (or more if you added tests). Never commit with failing tests.
