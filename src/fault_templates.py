...
async def _phase_jump(hil: Any, params: dict) -> dict:
    ...
    IEEE 2800 Section 7.3 mandates GFM IBR survival up to +/-25 deg. We allow the test
    bench to push slightly beyond (+/-30 deg) for boundary characterisation, but
    ...
    # IEEE 2800 Section 7.3 compliance band; allow small margin for boundary tests
    if not (-30.0 <= phase_step_deg <= 30.0):
        raise ValueError(
            f"phase_step_deg={phase_step_deg} outside +/-30 deg "
            "(IEEE 2800 Section 7.3 requires survival up to +/-25 deg)"
        )
...
FAULT_TEMPLATES["vsm_pref_step"] = FaultTemplate(
    name="vsm_pref_step",
    description="Apply Pref step on VSM inverter (IEEE 2800 Section 7.2.2).",
    required_params=(),
    apply=_vsm_pref_step,
)
FAULT_TEMPLATES["phase_jump"] = FaultTemplate(
    name="phase_jump",
    description="Apply phase angle step to grid source (IEEE 2800 Section 7.3).",
    required_params=(),
    apply=_phase_jump,
)
